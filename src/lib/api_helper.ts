...
export const SessionUser = createParamDecorator<SessionUser>((req) => req.user)

export const SessionPhoneNumber = createParamDecorator<SessionPhoneNumber>((req) => req.number)
...
export const NextAuthNumberGuard = createMiddlewareDecorator(
  async (req: NextApiRequest, _res: NextApiResponse, next: NextFunction) => {
    ...
    const phone_number = await getPrisma().phoneNumber.findUnique({
      where: { number: token.payload.number as string },
      include: { groups: { include: { group: true } } },
    })
    if (!phone_number) {
      throw new UnauthorizedException()
