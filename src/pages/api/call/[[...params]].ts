...
import {
    Body, Catch, createHandler, DefaultValuePipe, Get,
    NotFoundException, Param, ParseNumberPipe, Post,
    Query, SetHeader, Request, Header,
...
class CallRouter {
    ...
    @Post('/device')
    @SetHeader('Cache-Control', 'nostore')
    async add_device(@SessionPhoneNumber() number: SessionPhoneNumber, @Body() body: AddDeviceInput): Promise<ApiResponse<{}>> {
    ...
    public async contacts(
        ...
        const phone_number = await getPrisma().phoneNumber.findUnique({ where: { number: number.number }, include: { groups: true } })
        if(!phone_number) {
    ...
    public async make_call(
        ...
        let dest_number = await getPrisma().phoneNumber.findUnique({
            where: { number: dest },
            include: { push_tokens: { where: { active: true } }, alias_for: { include: { groups: true, push_tokens: { where: { active: true} } } }, groups: true },
        })
    ...
    async call_invite(
        ...
        await getPrisma().streamSession.createMany({
            data: [
                { call_id: invite_call_id, room_id: room.id, number: dest },
            ]
        })
    ...
    async append_hook_log(call_id: string, room_id: string, number: string, body: CallHookInput, success: boolean, error?: string, user_agent?: string, remote_ip?: string) {
    ...
    async call_hook(
        ...
        console.log('hook', call_id, number, remote_ip, ua, body);
        const call = await getPrisma().call.findUnique({
            where: { id: call_id },
            include: {
                from: {
                ...
                to: {
                    include: { push_tokens: { where: { active: true } }, alias_for: { include: { push_tokens: { where: { active: true }} } } }
                }
            }
        })
        if (!call) {
        ...
        switch (body.action) {
            case CallHookAction.Cancel: {
                const update_success = await this.update_call_state_transaction(call_id, [CallState.RINGING, CallState.WAITING], CallState.CANCELED);
                if(call.to && update_success) {
                    ...
                    this.append_hook_log(call_id, call.room_id, number, body, false, "No call with ringing or waiting state", ua, remote_ip)
                }
                break;
            }
